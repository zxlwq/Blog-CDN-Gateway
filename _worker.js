...
export default {
	async fetch(request, env) {
		...
		const 网站图标 = env.ICO || 'https://blog.wedp.dpdns.org/jpg/1.webp';
		const 网站头像 = env.PNG || 'https://blog.wedp.dpdns.org/jpg/1.webp';
		...
		if (url.pathname.toLowerCase() == '/ads.txt') {
			...
			// 先测速，不加载背景图片
			let img = 'https://blog.wedp.dpdns.org/jpg/background.webp'; // 默认图片
			if (env.IMG) {
